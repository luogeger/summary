...
<div class="row-fluid operation">
	...
	<div class="col-md-6 col-sm-6">
		<input type="hidden" id="securityDeviceId-edit"/>
...
<script>
	...
	$(function(){
		...
		setTimeout("securityDetail()",200);
		;
	});
	function securityDetail(){
		$.getJSON("security/device/detail?securityDeviceId="+securityId,function(r){
			if(r.code == 200){
				var securityDevice = r.result.NetworkDeviceVO;
				$("#securityDeviceId-edit").val(securityDevice.securityDeviceId);
				$("#assetType-edit").val(securityDevice.assetTypeName);
				$("#assetTypeId-edit").val(securityDevice.assetTypeId);
				$("#assetCode-edit").val(securityDevice.assetCode);
				$("#remark-edit").val(securityDevice.remark);
				$("#ipv4-edit").val(securityDevice.ipv4);
				$("#bandwidth-edit").val(securityDevice.bandwidth);
				$("#datacenter-edit").selectpicker("val",securityDevice.dataCenterId);
				$("#serviceStatus-edit").selectpicker("val",securityDevice.serviceStatus);
				$("#systemStatus-edit").selectpicker("val",securityDevice.systemStatus);
				$("#manufacturer-edit").val(securityDevice.manufacturerName);
				$("#manufacturerId-edit").val(securityDevice.manufacturerId);
				$("#price-edit").val(securityDevice.price);
				$("span.unit").text(securityDevice.units);
				$("#purchaseDate-edit").val(moment(securityDevice.purchaseDate).format("YYYY-MM-DD"));
				$("#expirationDate-edit").val(moment(securityDevice.expirationDate).format("YYYY-MM-DD"));
				$("#model-edit").val(securityDevice.modelName);
				$("#modelId-edit").val(securityDevice.modelId);
				$("#label-edit").selectpicker("val",securityDevice.labelId);
				$("#cabinet-edit").selectpicker("val",securityDevice.cabinetId);
				$("#unitStart-edit").val(securityDevice.unitStart);
				$("#unitEnd-edit").val(securityDevice.unitEnd);
				$(".selectpicker").selectpicker("refresh");
				loadConfiguration(securityDevice.assetTypeId,"assetTypeId-edit",securityDevice.data);
	...
	function saveInfo(){
		var securityDeviceId = $("#securityDeviceId-edit").val();
		...
		var data = {securityDeviceId:securityDeviceId, assetTypeId:assetTypeId, assetCode:assetCode, remark:remark, ipv4:ipv4, bandwidth:bandwidth, 

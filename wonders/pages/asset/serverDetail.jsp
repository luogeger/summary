...
<div class="row-fluid operation">
	...
	<div class="col-md-6 col-sm-6">
		<input type="hidden" id="serverDeviceId-edit"/>
	...
	<div class="col-md-6 col-sm-6">
		<label class="col-md-3 col-sm-4 control-label col" for="monitor-btn">系统类型</label>
		<div class="col-md-9 col-sm-8 col">
			<select id="osType-edit" class="form-control input-sm selectpicker input-group-select">
            </select>
		</div>
	</div>
	<div class="col-md-6 col-sm-6">
		<label class="col-md-3 col-sm-4 control-label col" for="monitor-btn">系统版本</label>
		<div class="col-md-9 col-sm-8 col">
			<select id="osVersion-edit" class="form-control input-sm selectpicker input-group-select">
            </select>
		</div>
	</div>
	<div class="col-md-6 col-sm-6">
...
<script>
	...
	$(function(){
		...
		//资源池
		initPoolSelects("resourcePool-edit",{type:COMPUTE_POOL_TYPE});
		//服务状态
		...
		setTimeout("serverDetail()",300);
		;
	});
	function serverDetail(){
		$.getJSON("server-device/get?serverDeviceId="+serverId_ori,function(r){
			if(r.code == 200){
				var serverDevice = r.result.ServerDeviceVO;
				$("#serverDeviceId-edit").val(serverDevice.serverDeviceId);
				$("#assetType-edit").val(serverDevice.assetTypeName);
				$("#assetTypeId-edit").val(serverDevice.assetTypeId);
				$("#assetCode-edit").val(serverDevice.assetCode);
				$("#remark-edit").val(serverDevice.remark);
				$("#ipv4-edit").val(serverDevice.ipv4);
				$("#datacenter-edit").selectpicker("val",serverDevice.dataCenterId);
				$("#serviceStatus-edit").selectpicker("val",serverDevice.serviceStatus);
				$("#systemStatus-edit").selectpicker("val",serverDevice.systemStatus);
				$("#manufacturer-edit").val(serverDevice.manufacturerName);
				$("#manufacturerId-edit").val(serverDevice.manufacturerId);
				$("span.unit").text(serverDevice.units);
				$("#price-edit").val(serverDevice.price);
				$("#purchaseDate-edit").val(serverDevice.purchaseDate);
				$("#expirationDate-edit").val(serverDevice.expirationDate);
				$("#model-edit").val(serverDevice.modelName);
				$("#modelId-edit").val(serverDevice.modelId);
				$("#label-edit").selectpicker("val",serverDevice.labelId);
				$("#cabinet-edit").selectpicker("val",serverDevice.cabinetId);
				$("#unitStart-edit").val(serverDevice.unitStart);
				$("#unitEnd-edit").val(serverDevice.unitEnd);
				$(".selectpicker").selectpicker("refresh");
				loadConfigurationInfo(serverDevice.assetTypeId,serverDevice.data);
	...
	function back(){
		loadMainPage("itam-content","pages/asset/serverList.jsp");
	}
	...
	function saveInfo(){
		var serverDeviceId = $("#serverDeviceId-edit").val();
		...
		var configurationInfo = $(".configurationInfo input");
		...
		var data = {serverDeviceId:serverDeviceId, assetTypeId:assetTypeId, assetCode:assetCode, remark:remark, ipv4:ipv4, 
